    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:lambda:eu-west-2:*:function:hnb-*"
      },
      {
        Sid    = "RedshiftDataAPI"
        Effect = "Allow"
...
resource "aws_iam_instance_profile" "airflow" {
  name = "HNBAirflowInstanceProfile"
  role = aws_iam_role.airflow_ec2_role.name
}
...
resource "aws_instance" "airflow" {
  ...
  user_data = <<-EOF
    ...
    git clone https://github.com/TimiOlayinka/hnb-data-pipeline.git /opt/hnb-repo
    cp /opt/hnb-repo/airflow/dags/*.py $AIRFLOW_HOME/dags/
    cp /opt/hnb-repo/airflow/config/*.yml $AIRFLOW_HOME/config/
    ...
    cp /opt/hnb-repo/airflow/dags/*.py /opt/airflow/dags/
    cp /opt/hnb-repo/airflow/config/*.yml /opt/airflow/config/
    ...
    Environment=REDSHIFT_IAM_ROLE=arn:aws:iam::332779204498:role/HNBDataGeneratorRole
    User=airflow
