...
------------------------------------------------------------------------------------------------------------------------
-- plt_products
-- Deduplicated PrettyLittleThing products from plt_products_history
------------------------------------------------------------------------------------------------------------------------
...
DROP TABLE IF EXISTS "rdl_salesforce_commerce".plt_products;

CREATE TABLE "rdl_salesforce_commerce".plt_products
DISTSTYLE KEY DISTKEY (product_id)
SORTKEY (product_id)
AS
WITH history AS (
    SELECT product_id, display_name AS product_name, c_color AS colour, primary_category_id AS category,
    ...
    FROM "rdl_salesforce_commerce".plt_products_history
),
dedup AS (
