--@variable "rdl_meta_ads" ${SCHEMA_RDL_META_ADS}

------------------------------------------------------------------------------------------------------------------------
-- campaign_insights
-- Deduplicated Meta Ads campaign insights from campaign_insights_history
-- Pattern: history → dedup (ROW_NUMBER) → SELECT WHERE rnk = 1
------------------------------------------------------------------------------------------------------------------------

SET enable_case_sensitive_identifier TO FALSE;

BEGIN TRANSACTION;

DROP TABLE IF EXISTS "rdl_meta_ads".campaign_insights;

CREATE TABLE "rdl_meta_ads".campaign_insights
DISTSTYLE KEY DISTKEY (campaign_id)
SORTKEY (date_start)
AS
WITH history AS (
    SELECT campaign_id, data."campaign_name", data."objective", data."account_id",
           publisher_platform,
           date_start, data."spend", data."impressions", data."clicks",
           data."reach", data."frequency",
           data."cpc", data."cpm", data."ctr",
           data."outbound_clicks"[0].value                  AS outbound_clicks,
           data."outbound_clicks_ctr"[0].value              AS outbound_clicks_ctr,
           data."video_p25_watched_actions"[0].value        AS video_p25_watched,
           data."video_p50_watched_actions"[0].value        AS video_p50_watched,
           data."video_p75_watched_actions"[0].value        AS video_p75_watched,
           data."video_p100_watched_actions"[0].value       AS video_p100_watched,
           brand,
           ingest_date, REPLACE(SPLIT_PART(ingest_time, '+', 1), 'T', ' ')::TIMESTAMP AS ingest_ts
    FROM "rdl_meta_ads".campaign_insights_history
),
dedup AS (
    SELECT *, ROW_NUMBER() OVER (
        PARTITION BY campaign_id, date_start, publisher_platform, brand
        ORDER BY ingest_date DESC, ingest_ts DESC
    ) AS rnk
    FROM history
)
SELECT
    campaign_id                                     :: VARCHAR(50)      AS campaign_id,
    campaign_name                                   :: VARCHAR(5000)    AS campaign_name,
    objective                                       :: VARCHAR(50)      AS objective,
    account_id                                      :: VARCHAR(50)      AS account_id,
    publisher_platform                              :: VARCHAR(50)      AS publisher_platform,
    date_start                                      :: DATE             AS date_nk,
    spend                                           :: DECIMAL(18,4)    AS spend,
    impressions                                     :: BIGINT           AS impressions,
    clicks                                          :: BIGINT           AS clicks,
    reach                                           :: BIGINT           AS reach,
    frequency                                       :: DECIMAL(18,4)    AS frequency,
    cpc                                             :: DECIMAL(18,4)    AS cpc,
    cpm                                             :: DECIMAL(18,4)    AS cpm,
    ctr                                             :: DECIMAL(18,4)    AS ctr,
    outbound_clicks                                 :: DECIMAL(18,4)    AS outbound_clicks,
    outbound_clicks_ctr                             :: DECIMAL(18,4)    AS outbound_clicks_ctr,
    video_p25_watched                               :: DECIMAL(18,4)    AS video_p25_watched,
    video_p50_watched                               :: DECIMAL(18,4)    AS video_p50_watched,
    video_p75_watched                               :: DECIMAL(18,4)    AS video_p75_watched,
    video_p100_watched                              :: DECIMAL(18,4)    AS video_p100_watched,
    brand                                           :: VARCHAR(50)      AS brand,
    ingest_date, ingest_ts
FROM dedup WHERE rnk = 1
;

COMMIT TRANSACTION;
