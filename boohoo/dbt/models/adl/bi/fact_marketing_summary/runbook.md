# Runbook: fact_marketing_summary
...
FROM {{ ref('fact_marketing_summary') }};
```

## Recovery steps
1. Check Airflow DAG logs for the failing task
2. Verify source data exists in S3 (`s3://data-lake/inbox/`)
3. Re-run the specific model: `dbt run --select fact_marketing_summary`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select fact_marketing_summary+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select fact_marketing_summary`
