...
WITH meta AS (
    SELECT date_nk, brand, brand_tier,
           ...
           SUM(clicks) AS clicks, SUM(reach) AS reach, 0 AS conversions,
           0::NUMERIC AS revenue, 0 AS sessions
...
google AS (
    SELECT date_nk, brand, brand_tier,
           ...
           SUM(conversions)::INT, SUM(conversions_value), 0
...
tiktok AS (
    SELECT date_nk, brand, brand_tier,
           ...
           SUM(conversion)::INT, 0::NUMERIC, 0
    FROM {{ ref('fact_tiktok_ad_insights') }}
    GROUP BY 1, 2, 3
),
email AS (
    SELECT date_nk, brand, brand_tier,
           'Email' AS channel, 'Owned' AS channel_type,
           0::NUMERIC, SUM(delivered)::BIGINT, SUM(clicked)::BIGINT, SUM(delivered)::BIGINT,
           SUM(converted)::INT, SUM(revenue), 0
    FROM {{ ref('fact_email_engagement') }}
    GROUP BY 1, 2, 3
),
influencer AS (
    SELECT date_nk, brand, brand_tier,
           'Influencer' AS channel, 'Earned' AS channel_type,
           SUM(cost), SUM(impressions), SUM(link_clicks)::BIGINT, SUM(reach),
           SUM(conversions)::INT, SUM(revenue_attributed), 0
    FROM {{ ref('fact_influencer_performance') }}
    GROUP BY 1, 2, 3
),
ga4 AS (
    SELECT date_nk, brand, brand_tier,
           'Website (GA4)' AS channel, 'All Traffic' AS channel_type,
           0::NUMERIC, 0::BIGINT, 0::BIGINT, 0::BIGINT,
           SUM(transactions)::INT, SUM(revenue), COUNT(*) AS sessions
