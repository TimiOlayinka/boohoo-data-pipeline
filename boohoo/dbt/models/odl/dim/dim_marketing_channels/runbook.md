# Runbook: dim_marketing_channels
...
SELECT COUNT(*) FROM {{ ref('dim_marketing_channels') }};

-- Check for orphan keys (fact without dimension)
-- Adapt the JOIN below for this specific model
SELECT COUNT(*) FROM {{ ref('dim_marketing_channels') }} f
...
3. Re-run the specific model: `dbt run --select dim_marketing_channels`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select dim_marketing_channels+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select dim_marketing_channels`
