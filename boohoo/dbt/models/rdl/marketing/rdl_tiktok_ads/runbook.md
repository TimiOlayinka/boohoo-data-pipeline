...
- Null values in natural key columns (ad_id, stat_time_day, brand)
- Ingestion timestamp parsing failure (ISO format change)

## Diagnostics
```sql
-- Check latest ingestion date
SELECT MAX(ingest_date), COUNT(*) FROM {{ ref('rdl_tiktok_ads') }}
WHERE is_latest;

-- Check for null natural keys
SELECT COUNT(*) FROM {{ ref('rdl_tiktok_ads') }}
WHERE ad_id IS NULL OR stat_time_day IS NULL OR brand IS NULL;
...
3. Re-run the specific model: `dbt run --select rdl_tiktok_ads`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select rdl_tiktok_ads+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select rdl_tiktok_ads`
