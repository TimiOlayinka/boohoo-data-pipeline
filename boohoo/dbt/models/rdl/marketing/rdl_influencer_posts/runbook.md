...
- Null values in natural key columns (post_id, brand)
- Ingestion timestamp parsing failure (ISO format change)

## Diagnostics
```sql
-- Check latest ingestion date
SELECT MAX(ingest_date), COUNT(*) FROM {{ ref('rdl_influencer_posts') }}
WHERE is_latest;

-- Check for null natural keys
SELECT COUNT(*) FROM {{ ref('rdl_influencer_posts') }}
WHERE post_id IS NULL OR brand IS NULL;

-- Check version distribution
SELECT version_count, COUNT(*) 
FROM {{ ref('rdl_influencer_posts') }}
...
3. Re-run the specific model: `dbt run --select rdl_influencer_posts`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select rdl_influencer_posts+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select rdl_influencer_posts`
