# Runbook: boohoo_products

## What this model supports
Boohoo Products — RDL layer model
...
SELECT MAX(ingest_date), COUNT(*) FROM {{ ref('boohoo_products') }}
WHERE is_latest;

-- Check for null natural keys
SELECT COUNT(*) FROM {{ ref('boohoo_products') }}
WHERE product_id IS NULL;

-- Check version distribution
SELECT version_count, COUNT(*) 
FROM {{ ref('boohoo_products') }}
...
3. Re-run the specific model: `dbt run --select boohoo_products`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select boohoo_products+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select boohoo_products`
