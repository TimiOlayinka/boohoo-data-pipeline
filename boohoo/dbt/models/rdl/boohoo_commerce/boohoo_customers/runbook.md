...
SELECT MAX(ingest_date), COUNT(*) FROM {{ ref('boohoo_customers') }}
WHERE is_latest;

-- Check for null natural keys
SELECT COUNT(*) FROM {{ ref('boohoo_customers') }}
WHERE customer_id IS NULL;

-- Check version distribution
SELECT version_count, COUNT(*) 
FROM {{ ref('boohoo_customers') }}
...
3. Re-run the specific model: `dbt run --select boohoo_customers`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select boohoo_customers+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select boohoo_customers`
