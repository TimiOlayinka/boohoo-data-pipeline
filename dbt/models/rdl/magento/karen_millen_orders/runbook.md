# Runbook: karen_millen_orders

## What this model supports
Karen Millen Orders — RDL layer model
...
SELECT MAX(ingest_date), COUNT(*) FROM {{ ref('karen_millen_orders') }}
WHERE is_latest;

-- Check for null natural keys
SELECT COUNT(*) FROM {{ ref('karen_millen_orders') }}
WHERE order_id IS NULL;

-- Check version distribution
SELECT version_count, COUNT(*) 
FROM {{ ref('karen_millen_orders') }}
...
3. Re-run the specific model: `dbt run --select karen_millen_orders`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select karen_millen_orders+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select karen_millen_orders`
