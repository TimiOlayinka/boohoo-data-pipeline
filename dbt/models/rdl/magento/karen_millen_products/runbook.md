# Runbook: karen_millen_products

## What this model supports
Karen Millen Products — RDL layer model
...
- Null values in natural key columns (product_id)
- Ingestion timestamp parsing failure (ISO format change)

## Diagnostics
```sql
-- Check latest ingestion date
SELECT MAX(ingest_date), COUNT(*) FROM {{ ref('karen_millen_products') }}
WHERE is_latest;

-- Check for null natural keys
SELECT COUNT(*) FROM {{ ref('karen_millen_products') }}
WHERE product_id IS NULL;

-- Check version distribution
SELECT version_count, COUNT(*) 
FROM {{ ref('karen_millen_products') }}
...
3. Re-run the specific model: `dbt run --select karen_millen_products`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select karen_millen_products+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select karen_millen_products`
