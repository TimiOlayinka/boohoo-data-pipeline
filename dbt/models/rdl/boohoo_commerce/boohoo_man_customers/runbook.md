# Runbook: boohoo_man_customers

## What this model supports
Boohoo Man Customers — RDL layer model
...
- Null values in natural key columns (customer_id)
- Ingestion timestamp parsing failure (ISO format change)

## Diagnostics
```sql
-- Check latest ingestion date
SELECT MAX(ingest_date), COUNT(*) FROM {{ ref('boohoo_man_customers') }}
WHERE is_latest;

-- Check for null natural keys
SELECT COUNT(*) FROM {{ ref('boohoo_man_customers') }}
WHERE customer_id IS NULL;

-- Check version distribution
SELECT version_count, COUNT(*) 
FROM {{ ref('boohoo_man_customers') }}
...
3. Re-run the specific model: `dbt run --select boohoo_man_customers`
4. If schema drift: update the RDL model SQL and re-run
5. Verify downstream models: `dbt run --select boohoo_man_customers+`

## Escalation
data-team@boohoo.com

## Related checks
- dbt test: `dbt test --select boohoo_man_customers`
